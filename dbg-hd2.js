...
(async () => {
  ...
  console.log('seed:', seed.slice(0, 20));
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    const b = r.body.trim();
    if (!b || b.length < 20) { console.log(`#${i}: EMPTY/tiny (${b.length}B)`); continue; }
    if (/^[A-Za-z0-9_-]{50,}$/.test(b)) {
      try {
        const d = JSON.parse(await _dec(url, params, '634649', '0', seed, b));
        const srcs = d.sources.map(s=>`${s.quality}:${String(s.url||s.file||'').slice(0,60)}`).join('\n     ');
        console.log(`#${i}: DECRYPTED ${d.sources.length} sources\n     ${srcs}`);
        break;
      } catch (e) {
        console.log(`#${i}: payload but decrypt fail: ${e.message.slice(0,60)} (len=${b.length}, head=${b.slice(0,40)})`);
      }
    } else {
      console.log(`#${i}: NOT payload: len=${b.length} head=${b.slice(0,120).replace(/\n/g,' ')}`);
    }
  }
  console.log('DONE');
})();
